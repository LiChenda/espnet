...
# Copyright 2021  CASIA (author: Jing Shi)
# Apache 2.0
...
if [ -f $srcdir/spk2.scp ]; then
  utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/spk2.scp >$destdir/spk2_tmp.scp
fi

if [ -f $srcdir/noise1.scp ]; then
  if [ ! $train_or_valid = '-' ]; then
    utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/noise1.scp >$destdir/noise1_tmp.scp
  else
    utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/noise1.scp >$destdir/noise1.scp
  fi
else
  if [ ! $train_or_valid = '-' ]; then
    cat ${destdir}/spk1_tmp.scp | awk '{print $1 " local/30s_silence.wav"}' > $destdir/noise1_tmp.scp
  else
    cat ${destdir}/spk1_tmp.scp | awk '{print $1 " local/30s_silence.wav"}' > $destdir/noise1.scp
  fi
fi

# if [ -f $srcdir/noise2.scp ]; then
  # utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/noise2.scp >$destdir/noise2_tmp.scp
# else
  # cat s${destdir}/spk2_tmp.scp | awk '{print $1 " local/30s_silence.wav"}' > $destdir/noise2_tmp.scp
# fi
